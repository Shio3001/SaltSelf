...
export class SeatCanvasComponent extends React.Component {
  constructor(props) {
    ...
    this.draw_width = () => {
      return Math.max(this.props.seat_lenght_x * this.seat_fill_width, 400);
    };
    this.draw_height = () => {
      return (
        this.props.seat_lenght_y * this.seat_fill_height +
        this.top_text_area +
        this.bottom_text_area
      );
    };
    this.toImage = () => {
        const png = document.getElementById("seat_view_canvas").toDataURL();
  ...
  componentDidMount() {
    const view_canvas_componentDidMount =
      document.getElementById("seat_view_canvas");
    view_canvas_componentDidMount.height = 10;
  }

  componentDidUpdate() {
    const view_canvas_init = document.getElementById("seat_view_canvas");
    view_canvas_init.height = this.draw_height();
    const context_init = view_canvas_init.getContext("2d");
    context_init.fillStyle = "rgba(220,220,220,1)";
    context_init.fillRect(
      0,
      0,
      view_canvas_init.width,
      view_canvas_init.height
    );
    ...
    context_init.fillRect(
      0,
      view_canvas_init.height - this.bottom_text_area,
      view_canvas_init.width,
      this.bottom_text_area
    );
    ...
    context_init.fillText(
      this.props.train_title,
      view_canvas_init.width / 2,
      40 + 10
    );

    context_init.font = "20px sans-serif";
    context_init.fillText(
      this.props.train_memo,
      view_canvas_init.width / 2,
      this.top_text_area / 2 + 10
    );

    context_init.font = "40px sans-serif";
    context_init.fillText(
      this.props.district_Upper + " 方面",
      view_canvas_init.width / 2,
      this.top_text_area - 10
    );
    context_init.fillText(
      this.props.district_Lower + " 方面",
      view_canvas_init.width / 2,
      view_canvas_init.height - 10
    );
    ...
    const seat_draw = (seat_x, seat_y, seat_name) => {
      ...
      seat_text.fillText(
        seat_name,
        fill_x + this.seat_fill_width / 2,
        fill_y + this.seat_fill_height * 0.63 + this.top_text_area
      );
  ...
  render() {
    ...
    console.log(
      "this.props.seat_lenght",
      this.props.seat_lenght_x,
      this.props.seat_lenght_y
    );
